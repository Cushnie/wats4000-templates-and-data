<template>
  <div class="results">
    ...
    <ul>
      <li class="movie-item" v-for="(result,id) in results" :key="id">
        ...
        <div class="ratings">
          <span class="rating-category critics-choice" v-if="result.vote_average > 8">Freaking epic!</span>

          <span
            class="rating-category well-liked"
            v-if="result.vote_average > 7 && result.vote_average <= 8"
          >Worth a couple of hours of your time.</span>

          <span class="rating-category stinker" v-if="result.vote_average < 7">Meh.</span>
 ...
 votes
        ...
        <p
          class="release-date"
        >Released: {{new Date(result.release_date).toLocaleDateString('en-US')}}</p>
